...
// Client-side shell for the admin tree. Lives behind the server
// layout in (admin)/layout.js — that server file is what owns the
// `metadata` export (browser tab title), since metadata can only be
// exported from server components. All interactive behavior — auth
// gates, theme initialization, sidebar collapse state — lives here.
//
// Theme: useAdminTheme is admin-only. It never touches
// document.documentElement — the wrapper div below adds className="dark"
// when the admin theme is dark. Tailwind `dark:` utilities scope to that
// wrapper. The student dashboard is unaffected by the admin's theme
