...
// exercise it deterministically per tier.
function computeVisibleLinkKeys(tier) {
  const navLinks = [
...
describe("sidebar Admissions entry — tier gating", () => {
  test("free tier: sidebar does not include /admissions", () => {
    expect(computeVisibleLinkKeys("free")).not.toContain("/admissions");
  });

  test("scholar tier: sidebar does not include /admissions", () => {
    expect(computeVisibleLinkKeys("scholar")).not.toContain("/admissions");
  });

  test("elite tier: sidebar includes /admissions immediately after /study-plan", () => {
    const links = computeVisibleLinkKeys("elite");
  ...
  test("vip tier: sidebar includes /admissions immediately after /study-plan", () => {
    const links = computeVisibleLinkKeys("vip");
    const idx = links.indexOf("/admissions");
    expect(idx).toBeGreaterThan(0);
    expect(links[idx - 1]).toBe("/study-plan");
  });
  ...
  test("undefined / null / unknown tier: sidebar does not include /admissions", () => {
    expect(computeVisibleLinkKeys(undefined)).not.toContain("/admissions");
    expect(computeVisibleLinkKeys(null)).not.toContain("/admissions");
    expect(computeVisibleLinkKeys("premium")).not.toContain("/admissions");
  });
});
